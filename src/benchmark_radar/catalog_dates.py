...
DEFAULT_DATES_PATH = Path("data/catalog/benchmark_dates.yml")
RELEASE_BASES = {
    "paper_first_version",
    "paper_publication",
...
def load_benchmark_dates(
    ...
    try:
        ...
        raise CatalogError(f"{path}: cannot read benchmark dates: {exc}") from exc
    if not isinstance(payload, dict) or payload.get("schema_version") != 1:
        raise CatalogError(f"{path}: expected benchmark dates schema_version 1")
    facts = payload.get("benchmarks")
    if not isinstance(facts, dict):
        raise CatalogError(f"{path}: benchmarks must be a mapping")
    keys = {row["key"] for row in records}
    validated = {}
    for key, fact in facts.items():
        if key not in keys:
            raise CatalogError(f"{path}: unknown benchmark key {key!r}")
        if not isinstance(fact, dict):
            raise CatalogError(f"{path}: {key}: expected a benchmark-date record")
        field = "released" if "released" in fact else "first_score_reported_at"
        if "released" in fact and "first_score_reported_at" in fact:
            raise CatalogError(f"{path}: {key}: each fact must date one event")
        ...
        try:
            ...
            valid = False
        if not valid:
            raise CatalogError(f"{path}: {key}: {field} must be an exact ISO date")
        bases = RELEASE_BASES if field == "released" else {"score_publication"}
        if fact.get("basis") not in bases:
            raise CatalogError(f"{path}: {key}: unsupported release-date basis")
        ...
        if (
            ...
            raise CatalogError(f"{path}: {key}: a primary-source HTTPS URL is required")
        if not isinstance(fact.get("note"), str) or not fact["note"].strip():
            raise CatalogError(f"{path}: {key}: explain which event the evidence dates")
